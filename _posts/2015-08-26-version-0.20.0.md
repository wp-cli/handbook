...
Use the `--regex` flag to `wp search-replace` to use regex in your operation. And, with the `--verbose` flag, you'll see each column with affected rows as the operation is performed:
...
We've [released](https://github.com/wp-cli/wp-cli/releases) v0.17.2, v0.18.1, and v0.19.3 with the bug fix. The cache key was changed as well, so prior invalid WordPress archives are silently ignored.
