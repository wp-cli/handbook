...
Consider the following ([#2523](https://github.com/wp-cli/wp-cli/issues/2523)):
...
If you had to complete this task using the WordPress network admin, it would take you hours, if not days, to complete. Simply _writing_ the WP-CLI command for this issue took me only three minutes.

WP-CLI generally follows the 80/20 rule when deciding whether to introduce a new feature. If it could be useful to most people, then maybe; otherwise, probably not. But those 20% shouldn't be left with the short stick, particularly when it comes to the really helpful commands.

Relevant pull requests for `wp package` include [#2442](https://github.com/wp-cli/wp-cli/pull/2442),
